...
const homeBottomCreative = '<a class="home-footer-banner" href="article.html?slug=arena-breakout-infinite-pc-guide" aria-label="Explore Arena Breakout: Infinite coverage"><img src="affiliate-assets/arena-breakout-footer-994x102.png" alt="Arena Breakout: Infinite tactical extraction shooter promotion"></a>';
if (document.body.dataset.page === 'home' && !document.querySelector('.home-footer-ad')) { const footer=document.querySelector('footer'); if (footer) footer.insertAdjacentHTML('beforebegin','<div class="wrap home-footer-ad">'+homeBottomCreative+'</div>'); }if (document.body.dataset.page === 'article') { const main=document.querySelector('.article-page'); main.insertAdjacentHTML('afterbegin','<div class="ad-band">'+creativeBanner+'</div>'); const anchor=document.createElement('div'); anchor.className='ad-anchor'; anchor.innerHTML=bottomCreative; anchor.querySelector('button').onclick=()=>anchor.remove(); main.appendChild(anchor); window.addEventListener('scroll',()=>{if(window.scrollY>300) anchor.classList.add('is-visible')},{passive:true}); }
