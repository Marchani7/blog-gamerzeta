...
if (document.body.dataset.page === 'article' && slug) { const coverImage=document.querySelector('.article-cover-image'); const coverSrc=articleImages[slug] || gameImages[document.querySelector('[data-game]')?.textContent] || 'gamerzeta-hero.png'; if(coverImage) coverImage.src=coverSrc; }
if (document.body.dataset.page === 'article' && slug === 'raid-shadow-legends-pc-roundup') { document.querySelector('.article-cover')?.classList.add('raid-roundup-cover'); }
...
const homeBottomCreative = '<a class="bottom-creative" href="article.html?slug=arena-breakout-infinite-pc-guide" aria-label="Explore Arena Breakout: Infinite coverage"><img class="bottom-creative-slide bottom-creative-slide-a" src="affiliate-assets/arena-breakout-banner.png" alt="Arena Breakout: Infinite tactical extraction artwork"><img class="bottom-creative-slide bottom-creative-slide-b" src="affiliate-assets/arena-breakout-300x250-no-cta.jpg" alt="" aria-hidden="true"><span class="bottom-creative-copy mono">ARENA BREAKOUT: INFINITE / PC GUIDE</span><span class="bottom-creative-cta">EXPLORE THE GUIDE</span></a><button type="button" aria-label="Close ad">×</button>';
if (document.body.dataset.page === 'home' && !document.querySelector('.home-ad-anchor')) { const anchor=document.createElement('div'); anchor.className='ad-anchor home-ad-anchor'; anchor.innerHTML=homeBottomCreative; anchor.querySelector('button').onclick=()=>anchor.remove(); document.body.appendChild(anchor); window.addEventListener('scroll',()=>{if(window.scrollY>320) anchor.classList.add('is-visible')},{passive:true}); }if (document.body.dataset.page === 'article') { const main=document.querySelector('.article-page'); main.insertAdjacentHTML('afterbegin','<div class="ad-band">'+creativeBanner+'</div>'); const anchor=document.createElement('div'); anchor.className='ad-anchor'; anchor.innerHTML=bottomCreative; anchor.querySelector('button').onclick=()=>anchor.remove(); main.appendChild(anchor); window.addEventListener('scroll',()=>{if(window.scrollY>300) anchor.classList.add('is-visible')},{passive:true}); }
